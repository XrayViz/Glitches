...
#pragma omp parallel for shared(cUCs, omegas, phis, gDiss)
  ...
  {
    double cUC = ucStart + (ucEnd-ucStart)*(double)iuc/(double)(numUC-1);
    double cell_a = cUC; 
    ...
    for (int iph=0; iph<numPh; iph++)
      ...
      {
        double omega = omStart + (omEnd-omStart)*(double)iom/(double)(numOm-1);
        double phi = phStart + (phEnd-phStart)*(double)iph/(double)(numPh-1);
  
        int doneN = CalcGlitches(omega, phi, numE, numFi, tolerance,
